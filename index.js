...
async function main() {
  ...
  // Start HTTP server for Railway (health checks and public access)
  ...
  app.get('/', (req, res) => {
    res.json({
      name: 'LodeStar Apollo3 MCP Server',
      version: '1.0.0',
      status: 'running',
      description: 'MCP server for LodeStar Apollo3 API - title insurance and closing cost calculations',
      endpoints: {
        mcp: 'Use via MCP protocol (stdio)',
        health: 'GET /',
        info: 'GET /info'
      },
      tools: [
        'lodestar_login',
        'lodestar_closing_cost_calculations',
        'lodestar_property_tax',
        'lodestar_get_endorsements',
        'lodestar_get_sub_agents',
        'lodestar_get_counties',
        'lodestar_get_townships',
        'lodestar_get_questions',
        'lodestar_geocode',
        'lodestar_get_session_status'
      ]
    });
  });
  ...
  app.get('/info', (req, res) => {
    res.json({
      server: 'LodeStar Apollo3 MCP Server',
      mcp_version: '1.0.0',
      api_version: '2.4.1',
      provider: 'LodeStarSS',
      documentation: 'https://github.com/your-repo/lodestar-mcp-server'
    });
  });
  ...
  // Only run MCP server if in stdio mode (not HTTP-only deployment)
  if (process.argv.includes('--mcp') || !process.env.PORT) {
